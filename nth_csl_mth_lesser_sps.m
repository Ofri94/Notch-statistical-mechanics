function [z,T,Sa,Sb,phase] = nth_csl_mth_lesser_sps(N,M,phi)

%this script is meant to solve numerically the partition function and sensitivity
%function of nth csl sites and mth sps sites notch gene, wehere 1 of the sps sites is composed of a canonical site and lesser site.
%alpha and beta ranges logarithmically from 0 to 5
%phi is set to 10 - affinity of lesser site is 10th of canonical
%this code assumes repressor presence completly inhibits transcription,
...
if N>=0 && M>0
    ...
    z=(1+A+b).^(N).*(1+(1+1/phi)*A+((C/phi)*A.^2)+(1+1/phi)*b+((b.^2)./phi)+(2*A.*b)./phi).^(M);% where 1 descibes no occupency, a is the
...
%     arrangements(ismember(arrangements,[M, 0, 0],'rows'),:)=[];%erasing the row which results in 1^M. it doesnt need to be included in the calc.
    ...
    for itr1=1:(num_of_sps_terms)%going through all of the terms from the sps part
        ...
        current_term=current_term_co.*(1.^powers(1)).*(((1+1/phi)*a).^powers(2)).*(((C/phi)*a.^2).^powers(3));%formula for the appropriate term from the sps region of the script
